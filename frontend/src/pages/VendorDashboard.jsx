  ...
  const getTerms = () => {
    ...
    if (type.startsWith('Hospital')) {
      ...
      return {
        ...
        catalogStatuses: ['Active', 'Full / Closed'],
      ...
      return {
        ...
        catalogStatuses: ['Open / Hiring', 'Closed / Filled'],
      ...
      return {
        ...
        catalogStatuses: ['Available', 'Booked'],
  ...
  const handleOpenAddItem = () => {
    ...
    if (defaultMain && COMPLETE_CAT_TAXONOMY[defaultMain]) {
      const foundSub = Object.keys(COMPLETE_CAT_TAXONOMY[defaultMain]).find(k => 
        COMPLETE_CAT_TAXONOMY[defaultMain][k].includes(vendorCategory)
      );
      if (foundSub) {
        subcatVal = foundSub;
      } else {
        subcatVal = Object.keys(COMPLETE_CAT_TAXONOMY[defaultMain])[0] || '';
        thirdCatVal = (COMPLETE_CAT_TAXONOMY[defaultMain][subcatVal] && COMPLETE_CAT_TAXONOMY[defaultMain][subcatVal][0]) || '';
      }
    }
                                ...
                                <tr key={order._id} className="border-b border-slate-200 dark:border-slate-800/60 hover:bg-slate-100/40 dark:hover:bg-slate-900/20 text-sm text-slate-700 dark:text-slate-200">
                                  {isJob ? (
                                    ...
                                    <>
                                      ...
                                      <td className="px-6 py-4 font-semibold text-slate-900 dark:text-white">
                                        <div>{order.memberName || order.customer_name || 'N/A'}</div>
                                        <div className="text-[10px] text-slate-500 font-normal mt-0.5">ID: {order.memberId || order.id || 'N/A'}</div>
                                      </td>
                                      {/* Service Type */}
                                      <td className="px-6 py-4 text-xs font-bold text-indigo-600 dark:text-indigo-400">
